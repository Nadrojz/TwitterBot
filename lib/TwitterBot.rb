...
def get_journalist(an_array_journalists)
array = an_array_journalists.sample(1)
end

# def qui permet de tweeter sur ton compte
def client_update(message, journalist)
  client = login_twitter
  journalist.each do |handle|
    client.update("#{message} #{handle}")
  end
end

def tweet_journalists(another_array_journalists)
  # Récupère le nom de 1 journalistes dans une liste
  random_journalist = get_journalist(another_array_journalists)
  # Tweet un msg au journliste
  client_update('Merci pour votre dernier article #bonjour_monde', random_journalist )
end

def search_like
  client = login_twitter
  client.search('#bonjour_monde').take(10).collect do |tweet|
...
def search_follow
  client = login_twitter
  client.search('#bonjour_monde').take(2).collect do |tweet|
